...
public enum BlockingValueComponentProvider implements IEntityComponentProvider {
    INSTANCE;
    private static final Identifier FULL = BlockingValueJade.heartSprite("blocking_value_full");
    private static final Identifier HALF = BlockingValueJade.heartSprite("blocking_value_half");
    private static final int TEXT_THRESHOLD = 20;

    @Override
    public void appendTooltip(ITooltip tooltip, EntityAccessor accessor, IPluginConfig config) {
        if (!config.get(BlockingValueJade.UID)) {
            return;
        }

        float amount = accessor.getServerData().getFloatOr(BlockingValueJade.TAG_AMOUNT, 0.0F);
        if (amount <= 0.0F) {
            return;
        }
