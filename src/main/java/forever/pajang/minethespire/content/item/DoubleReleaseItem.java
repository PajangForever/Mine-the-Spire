...
public class DoubleReleaseItem extends Item {
    public static final int MAX_ENERGY = 128;
    private static final int BAR_COLOR = 0x3FA7FF;

    public DoubleReleaseItem(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResult use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide()) {
            if (!ChargeBallManager.get(player).tryDoubleRelease()) {
    ...
    private static int getEnergy(ItemStack stack) {
        Integer energy = stack.get(ModDataComponents.DOUBLE_RELEASE_ENERGY.get());
        return energy == null ? 0 : Mth.clamp(energy, 0, MAX_ENERGY);
    }

    private static void setEnergy(ItemStack stack, int energy) {
        stack.set(ModDataComponents.DOUBLE_RELEASE_ENERGY.get(), Mth.clamp(energy, 0, MAX_ENERGY));
    }
}
